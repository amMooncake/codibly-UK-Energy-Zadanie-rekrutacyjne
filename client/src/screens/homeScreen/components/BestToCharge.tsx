...
export default function BestTimeToCharge({ bestWindowData }: { bestWindowData: BestWindow | null }) {
  ...
  return (
    <Card className="w-full max-w-md">
      <CardHeader className="flex flex-row items-center space-x-4 pb-2">
        ...
        <div>
          <p className="text-2xl font-bold text-green-600">{bestWindowData?.averageEcoPercent}% Clean</p>
        </div>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        <div className="flex items-center gap-3 text-sm">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-xs text-muted-foreground">{bestWindowData?.from.substring(0,10)}</span>
          </div>
        </div>
